...
void Back_Week3_DrawBG(StageBack *back)
{
	...
	for (size_t i = 0; i < COUNT_OF(roof_piece); i++, roof_p++)
	{
		...
		Stage_DrawTex(&this->tex_back2, &roof_p->src, &roof_dst, stage.camera.bzoom, stage.camera.angle);
	...
	Stage_DrawTex(&this->tex_back2, &roof_fillsrc, &roof_fill, stage.camera.bzoom, stage.camera.angle);
	...
	else
	{
		...
		for (int i = 0; i < 24; i++, train_dst.x += train_dst.w)
		{
			if (train_dst.x >= (SCREEN_WIDTH2 << FIXED_SHIFT) || train_dst.x <= -(train_dst.w + (SCREEN_WIDTH2 << FIXED_SHIFT)))
				continue;
			Stage_DrawTex(&this->tex_back4, &train_src, &train_dst, stage.camera.bzoom, stage.camera.angle);
		}
	}
